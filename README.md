...
- **Speech stream** — audio recordings are preprocessed through Voice Activity Detection (VAD), spectral denoising, and resampling to 22,050 Hz. Mel-Frequency Cepstral Coefficients (MFCCs) are extracted with 40 coefficients per frame, and feature matrices are standardised to a fixed length of 500 frames.
- **EEG stream** — brain signals are band-pass filtered, cleaned via Independent Component Analysis (ICA) for artifact removal (muscle movements, eye blinks), z-score normalised, and segmented into overlapping 1-second windows (50% overlap) for frequency-domain feature extraction.

**Synchronisation via DTW:** A two-stage alignment approach first aligns timestamps by stimulus presentation times, then applies Dynamic Time Warping (DTW) to compensate for non-linear temporal variations. DTW was chosen over Connectionist Temporal Classification (CTC) — which is better suited for single-modality variable-length sequences — and over temporal convolutions, which require significantly more computational resources and complex training. The fused feature vector is then processed by a Transformer module that acts as a meta-learner, capturing long-range inter-modal dependencies via self-attention.
...
│   │   └── fusion_model.py    # TMNet multimodal fusion (Transformer-based)
│   ├── utils/
